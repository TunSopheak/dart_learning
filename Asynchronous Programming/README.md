...
ដើម្បីឱ្យកូដ Asynchronous មើលទៅងាយស្រួលអាន និងមានលក្ខណៈដូចកូដធម្មតា (Sequential) យើងนิยมប្រើប្រាស់ keyword **`async`** និង **`await`** ជំនួសឱ្យការប្រើ `.then()`។

```dart
// Function សម្រាប់ទាញយកទិន្នន័យខ្សែកាប (จำลอง API Request)
...
# មេរៀនទី ៤៖ ការจัดการ Error (Try-Catch with Async/Await)

ពេលដែលយើងទាញយកទិន្នន័យពី Network ឬ Database វាអាចនឹងមាន Error កើតឡើង (ឧទាហរណ៍៖ អត់មានអተርណែត ឬរកមិនឃើញ Server)។ ដូចនេះ យើងត្រូវប្រើ **`try-catch`** ដើម្បីទប់ស្កាត់កំហុសទាំងនោះ។

```dart
Future<void> getUserProfile() async {
  try {
    // จำลองកូដដែលបង្កឱ្យមាន Error
